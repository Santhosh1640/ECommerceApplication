package com.santhuuu.microservices.api_gatewayy.routes;
...
@Configuration
public class Routes {
    //Function endpoint programming model
    ...
    public RouterFunction<ServerResponse> productServiceSwaggerRoute() {
        return route()
                .route(RequestPredicates.path("/aggregate/product-service/v3/api-docs").and(RequestPredicates.method(HttpMethod.GET)),
                        request -> ServerResponse.permanentRedirect(URI.create("http://localhost:8080/v3/api-docs")).build())
    ...
    public RouterFunction<ServerResponse> orderServiceSwaggerRoute() {
        return GatewayRouterFunctions.route("order_service_swagger")
                ...
                .filter(CircuitBreakerFilterFunctions.circuitBreaker("orderServiceSwaggerCircuitBreaker",
                        URI.create("forward:/fallbackRoute")))
                .filter(setPath("/api-docs")) //this will replace the whole path(/aggregate/product.....) with /api-docs
                .build();
    }
